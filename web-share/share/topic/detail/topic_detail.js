window.onload = function () {
  ...
  if (ua.match(/MicroMessenger/i) == "micromessenger") {
    ...
    if (ua.indexOf('android') > -1) {
      DL.href = "http://a.app.qq.com/o/simple.jsp?pkgname=shengang.szkingdom.android.phone";
      DL.target = "_blank";
...
$(function () {
  ...
  var APP_DOWN_URL = 'http://a.app.qq.com/o/simple.jsp?pkgname=shengang.szkingdom.android.phone'
  ...
  function requestTopicDetailWithID() {
    ...
    var url = `http://106.14.166.235/api/v1/mobile/topic/get_detail?topic_id=${topicID}&filter=1&cursor=0`
  ...
  function loadCommentWithData(data) {
    ...
    $('#owner-avatar').find('img').attr('src', `http://x-community.oss-cn-shanghai.aliyuncs.com/image/202010/fbfb5447cac0d392ef0c8c545eb29c00.jpg`)
    var content = `<p>${data.content}</p>`;
    $('#bbs-thread-content').html(content)
    $('#owner-name').html(author.username)
    var time_label = `${formatDate(data.update_at)}<span>评论 ${data.comment_count}</span>`
    ...
    $.each(data.comments, function () {
      ...
      commentsHtml += '</div>'
      commentsHtml += '<div class="hp-m-discuss-item-wrap-content"><div class="hp-m-discuss-item-wrap-content-container">'
      commentsHtml += `<p>${this.content}</p>`
      commentsHtml += '</div></div>'
      commentsHtml += '<div class="hp-m-discuss-item-wrap-footer">'
      commentsHtml += `<div class="discuss-time">${formatDate(this.create_at)}</div>`
